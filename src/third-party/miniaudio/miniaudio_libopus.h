...
inline MA_API ma_result ma_libopus_init_memory(const void* pData, size_t dataSize, const ma_decoding_backend_config* pConfig, const ma_allocation_callbacks* pAllocationCallbacks, ma_libopus* pOpus)
{
    ma_result result = MA_NOT_IMPLEMENTED;
...
#if !defined(MA_NO_LIBOPUS)
    {
        ...
        pOpus->of = op_open_memory(reinterpret_cast<const unsigned char*>(pData), dataSize, &libopusResult);
        if (pOpus->of == NULL) {
            /*switch (libopusResult) {
                case OP_EREAD:
                    assert(0);
                    break;
                case OP_EFAULT:
